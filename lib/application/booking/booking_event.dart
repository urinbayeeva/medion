...
abstract class BookingEvent with _$BookingEvent {
  ...
  const factory BookingEvent.fetchCategoryServices({required int id}) = _FetchCategoryServices;

  factory BookingEvent.getDoctorsTime({required List<int> id}) = _GetDoctorsTime;

  const factory BookingEvent.filterDoctorsBySpecialty({required String specialty}) = _FilterDoctorsBySpecialty;

  const factory BookingEvent.filterDoctorsByPrice({required int maxPrice}) = _FilterDoctorsByPrice;

  const factory BookingEvent.fetchThirdBookingServices({required List<int> request}) = _FetchThirdBookingServices;
