...
class SelectedServicesProvider extends ChangeNotifier {
  ...
  List<Service> getSelectedServices(int serviceId) =>
      _selectedServicesByService[serviceId] ?? [];
  ...
  void addService(int serviceId, Service service) {
    ...
    if (!_selectedServicesByService[serviceId]!.contains(service)) {
      ...
      _serviceHasDoctorSessionByService.putIfAbsent(
          serviceId, () => {})[service.id!] = false;
      notifyListeners();
    }
  }
  ...
  void markServiceHasDoctorSession(
      int serviceId, int serviceItemId, bool hasSession) {
...
class SelectedServiceIdsProvider extends ChangeNotifier {
  ...
  List<int> getSelectedServiceIds(int serviceId) =>
      _selectedServiceIdsByService[serviceId] ?? [];
  ...
  void addServiceId(int serviceId, int id) {
    ...
    if (!_selectedServiceIdsByService[serviceId]!.contains(id)) {
      ...
      _serviceDoctorSessionIdsByService.putIfAbsent(serviceId, () => {})[id] =
          null;
      notifyListeners();
    }
  }
