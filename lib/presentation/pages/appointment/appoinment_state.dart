...
class AppointmentState {
  static final ValueNotifier<List<Map<String, String>>> selectedAppointments = ValueNotifier([]);
  
  static void addAppointment(Map<String, String> appointment) {
    final existingIndex = selectedAppointments.value.indexWhere(
      (a) => a['serviceId'] == appointment['serviceId']
    );
    
    if (existingIndex != -1) {
      final newList = List<Map<String, String>>.from(selectedAppointments.value);
  ...
  static bool isTimeSlotTaken(String date, String time, String currentServiceId) {
